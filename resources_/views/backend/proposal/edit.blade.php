    ...
    <div class="row">
        <div class="col-xl">
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <form action="/proposal/editProses" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <select class="form-control" name="kelasi_id" id="kelasi_id" required>
                                        <option value="">-- Pilih --</option>
                                        @foreach ($kelas as $k)
                                            <option value="{{ $k->nama_kelas }}"
                                                {{ $k->nama_kelas == $proposal->kelas ? 'selected' : '' }}>{{ $k->nama_kelas }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label class="form-label" for="full_name">Jenis Proposal</label>
                                    <select class="form-control" name="jenis_proposal" id="jenis_proposal" required>
                                        <option value="">-- Pilih --</option>
                                        @foreach ($jenis_proposal as $l)
                                            <option value="{{ $l->jenis }}"
                                                {{ $l->jenis == $proposal->jenis ? 'selected' : '' }}>{{ $l->nama }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
